...
_VTBL_PTR = ctypes.POINTER(_PPVOID)
...
_ADDREF_FUNC = ctypes.WINFUNCTYPE(ctypes.c_ulong, ctypes.c_void_p)
_RELEASE_FUNC = ctypes.WINFUNCTYPE(ctypes.c_ulong, ctypes.c_void_p)
...
class ComPtr:
    ...
    def release(self) -> None:
        if not self.ptr:
            return
        _vtbl_invoke(self.ptr, 2, ctypes.c_ulong, ())
        self.ptr.value = None
...
def _vtbl_invoke(
    ...
    *args,
    ...
    if not this_ptr:
        raise WinRTError("Null COM pointer")
    vtbl = ctypes.cast(this_ptr, _VTBL_PTR).contents
    func_addr = vtbl[index]
    fn_type = _get_vtbl_fn_type(restype, argtypes)
...
_windows_create_string.restype = ctypes.c_long

_windows_delete_string = _combase.WindowsDeleteString
_windows_delete_string.argtypes = [ctypes.c_void_p]
_windows_delete_string.restype = ctypes.c_long
...
_ro_get_activation_factory.restype = ctypes.c_long

_ro_activate_instance = _combase.RoActivateInstance
_ro_activate_instance.argtypes = [ctypes.c_void_p, _PPVOID]
_ro_activate_instance.restype = ctypes.c_long
...
_ARG_HANDLER_EVENT_TOKEN = (ctypes.c_void_p, _PEVENT_TOKEN)
_ARG_UINT32_OUT_VOIDP = (ctypes.c_uint32, _PPVOID)
_ARG_UINT64_OUT_VOIDP = (ctypes.c_uint64, _PPVOID)
_ARG_CINT_OUT_VOIDP = (ctypes.c_int, _PPVOID)
_ARG_GUID_OUT_VOIDP = (GUID, _PPVOID)
_ARG_VOIDP_OUT_VOIDP = (ctypes.c_void_p, _PPVOID)
_ARG_VOIDP_CINT_OUT_VOIDP = (ctypes.c_void_p, ctypes.c_int, _PPVOID)
_ARG_EVENT_TOKEN = (EventRegistrationToken,)
_ARG_VOIDP = (ctypes.c_void_p,)
_ARG_CINT = (ctypes.c_int,)
_ARG_UINT32 = (ctypes.c_uint32,)
...
def get_activation_factory(runtime_class: str, iid) -> ComPtr:
    with HString(runtime_class) as class_id:
        out = ctypes.c_void_p()
        hr = _ro_get_activation_factory(class_id, ctypes.byref(iid), ctypes.byref(out))
...
def activate_instance(runtime_class: str) -> ComPtr:
    with HString(runtime_class) as class_id:
        out = ctypes.c_void_p()
        hr = _ro_activate_instance(class_id, ctypes.byref(out))
...
def hstring_to_str(value: ctypes.c_void_p | None) -> str:
    ...
    raw = _windows_get_string_raw_buffer(value, ctypes.byref(length))
    return ctypes.wstring_at(raw, length.value) if raw else ""


def _prop_int(ptr, index, argtypes, name):
    out = argtypes[0]._type_()
    hr = _vtbl_invoke(ptr, index, ctypes.c_long, argtypes, ctypes.byref(out))
...
def _prop_ptr(ptr, index, name):
    out = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, index, ctypes.c_long, _ARG_OUT_VOIDP, ctypes.byref(out))
    _check_hresult(hr, name)
    return out
...
def _add_evt(ptr, index, handler_ptr, name):
    ...
    hr = _vtbl_invoke(ptr, index, ctypes.c_long, _ARG_HANDLER_EVENT_TOKEN, handler_ptr, ctypes.byref(token))
    _check_hresult(hr, name)
    return token


def _remove_evt(ptr, index, token, name):
    hr = _vtbl_invoke(ptr, index, ctypes.c_long, _ARG_EVENT_TOKEN, token)
    _check_hresult(hr, name)
...
def asyncop_set_completed(p, handler_ptr):  # IAsyncOperation::put_Completed [6]
    hr = _vtbl_invoke(p, 6, ctypes.c_long, _ARG_VOIDP, handler_ptr)
...
def received_args_get_local_name(args_ptr: ctypes.c_void_p) -> str | None:  # RecvArgs::get_Advertisement [10]
    adv = ComPtr(_prop_ptr(args_ptr, 10, "RecvArgs.get_Advertisement"))
    if not adv.ptr:
        return None
    raw_name = ctypes.c_void_p()
    try:
        hr = _vtbl_invoke(adv.ptr, 8, ctypes.c_long, _ARG_OUT_VOIDP, ctypes.byref(raw_name))  # IBluetoothLEAdvertisement::get_LocalName [8]
...
def btle_statics_from_bluetooth_address_async(  # IBluetoothLEDeviceStatics::FromBluetoothAddressAsync [7]
    ...
    hr = _vtbl_invoke(
        ...
        ctypes.c_uint64(bluetooth_address),
...
def btle_device6_request_throughput_params(device_ptr: ctypes.c_void_p) -> None:
    ...
    d6 = ctypes.c_void_p()
    if _vtbl_invoke(device_ptr, 0, ctypes.c_long,
            _ARG_PGUID_PPVOID,
            ctypes.byref(IID_BLUETOOTH_LE_DEVICE6), ctypes.byref(d6)) < 0:
        return
    try:
        ...
        preset = ctypes.c_void_p()
        _vtbl_invoke(statics.ptr, 7, ctypes.c_long, _ARG_OUT_VOIDP, ctypes.byref(preset))
        statics.release()
        req = ctypes.c_void_p()
        _vtbl_invoke(d6, 8, ctypes.c_long,  # RequestPreferredConnectionParameters [8]
            _ARG_VOIDP_OUT_VOIDP,
            preset, ctypes.byref(req))
        if req.value:
            _vtbl_invoke(req, 2, ctypes.c_ulong, ())  # Release result
        _vtbl_invoke(preset, 2, ctypes.c_ulong, ())  # Release preset
    except Exception:
        pass
    finally:
        _vtbl_invoke(d6, 2, ctypes.c_ulong, ())  # Release Device6


def btle_device3_get_gatt_services_for_uuid_async(ptr, service_uuid):  # IBluetoothLEDevice3::GetGattServicesForUuidAsync [10]
    op = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, 10, ctypes.c_long, _ARG_GUID_OUT_VOIDP, service_uuid, ctypes.byref(op))
...
def gatt_service3_get_characteristics_async(ptr):  # IGattDeviceService3::GetCharacteristicsAsync [11]
    op = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, 11, ctypes.c_long, _ARG_OUT_VOIDP, ctypes.byref(op))
...
def vector_view_get_at(ptr, index):  # IVectorView::GetAt [6]
    out = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, 6, ctypes.c_long, _ARG_UINT32_OUT_VOIDP, ctypes.c_uint32(index), ctypes.byref(out))
...
def gatt_characteristic_get_uuid(ptr):  # IGattCharacteristic::get_Uuid [11]
    ...
    hr = _vtbl_invoke(ptr, 11, ctypes.c_long, _ARG_OUT_GUID, ctypes.byref(out))
...
def gatt_characteristic_write_value_with_option_async(ptr, buffer_ptr, write_option):  # IGattCharacteristic::WriteValueWithOptionAsync [17]
    op = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, 17, ctypes.c_long, _ARG_VOIDP_CINT_OUT_VOIDP,
                      buffer_ptr, ctypes.c_int(write_option), ctypes.byref(op))
...
def gatt_characteristic_write_cccd_async(ptr, cccd_value):  # IGattCharacteristic::WriteClientCharacteristicConfigurationDescriptorAsync [19]
    op = ctypes.c_void_p()
    hr = _vtbl_invoke(ptr, 19, ctypes.c_long, _ARG_CINT_OUT_VOIDP, ctypes.c_int(cccd_value), ctypes.byref(op))
...
def buffer_set_length(ptr, length):  # IBuffer::put_Length [8]
    hr = _vtbl_invoke(ptr, 8, ctypes.c_long, _ARG_UINT32, ctypes.c_uint32(length))
    _check_hresult(hr, "Buffer.put_Length")

def buffer_get_data_ptr(buffer_ptr: ctypes.c_void_p):
    ba = ctypes.c_void_p()
    hr = _vtbl_invoke(buffer_ptr, 0, ctypes.c_long,
        _ARG_PGUID_PPVOID,
        ctypes.byref(IID_IBUFFER_BYTE_ACCESS), ctypes.byref(ba))
    _check_hresult(hr, "QI IBufferByteAccess")
    try:
        data = ctypes.c_void_p()
        hr = _vtbl_invoke(ba, 3, ctypes.c_long,  # IBufferByteAccess::Buffer [3]
            _ARG_OUT_VOIDP,
            ctypes.byref(data))
        _check_hresult(hr, "BufferByteAccess.Buffer")
        return data
    finally:
        _vtbl_invoke(ba, 2, ctypes.c_ulong, ())  # Release


def buffer_to_bytes(buffer_ptr: ctypes.c_void_p) -> bytes:
    length = ctypes.c_uint32()
    hr = _vtbl_invoke(buffer_ptr, 7, ctypes.c_long, _ARG_OUT_UINT32, ctypes.byref(length))
...
class _ComDelegate:
    ...
    def _init_com(self, iid, invoke_fn) -> None:
        ...
        self._vtable = _VTable4(
            ctypes.cast(self._fn_qi, ctypes.c_void_p).value,
            ctypes.cast(self._fn_add_ref, ctypes.c_void_p).value,
            ctypes.cast(self._fn_release, ctypes.c_void_p).value,
            ctypes.cast(self._fn_invoke, ctypes.c_void_p).value,
        )
        self._lpVtbl = ctypes.cast(ctypes.byref(self._vtable), ctypes.c_void_p)
        self.ptr = ctypes.cast(ctypes.byref(self._lpVtbl), ctypes.c_void_p)
