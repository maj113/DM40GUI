...
from shared.device_registry import FAMILY_MAP
...
def probe_device_type(device, timeout: float = 6.0) -> str | None:
    """Connect, send discovery command, return a registered device type or None.
...
async def _probe(device, timeout, result):
    ...
    def on_notify(data: bytes):
        if data.startswith(_DISCOVERY_HEADER) and len(data) >= 7:
            family = FAMILY_MAP.get(data[5:7])
            if family:
                result[0] = family
...
class BleWorker:
    ...
    def __init__(
        ...
        *,
        ...
        self._cmd_requires_notify = cmd_requires_notify
    ...
    async def _main(self) -> None:
        ...
        cmd_requires_notify = self._cmd_requires_notify
                ...
                while not self._stopping:
                    ...
                    if read_ready:
                        ...
                        if cmd is not None and on_tx:
                            on_tx(payload)
                        if cmd is None or cmd_requires_notify:
                            read_ready = False
                        else:
                            io_event.set()
